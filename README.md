...
use_repo(
    ...
    # Add any additional Swift packages. The name of the Bazel external
    # repository for a Swift package is `swiftpkg_xxx` where `xxx` is the Swift
    # package identity, lowercase, with punctuation replaced by `hyphen`.
    # For example, the repository name for apple/swift-nio is
    # `swiftpkg_swift_nio`.
)
```
...
# whenever you add or remove source files from your project. The `swift_deps_info` repository is
generated by
gazelle(
